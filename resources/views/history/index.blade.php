    ...
    <div class="container ">
        ...
        @if ($latestHistory)
            <div class="card mb-4 shadow-sm border-0">
                ...
                <div class="card-body">
                    <p><strong>Kategori Ruminansia:</strong> {{ $latestHistory->category }}</p>
                    <p><strong>Penyakit yang Didiagnosa:</strong> {{ $latestHistory->disease->name ?? 'Tidak Diketahui' }}
                    </p>
                    ...
                    <ul class="ml-3">
                        @php
                            $selectedSymptoms = json_decode($latestHistory->selected_symptoms, true);
                            $symptomNames = \App\Models\Symptom::whereIn('id', array_keys($selectedSymptoms))
                                ->pluck('name')
                                ->toArray();
                        @endphp
                ...
                <div class="card mb-3 shadow-sm border-0">
                    <div class="card-header bg-light d-flex justify-content-between align-items-center">
                        <button class="btn btn-link text-dark text-left w-100 d-flex align-items-center" type="button"
                            ...
                            <i class="fa fa-plus mr-2"></i> <!-- Ikon accordion -->
                    ...
                    <div id="collapse{{ $index }}" class="collapse" aria-labelledby="heading{{ $index }}"
                        ...
                        <div class="card-body">
                            <p><strong>Kategori Ruminansia:</strong> {{ $history->category }}</p>
                            <p><strong>Tanggal Diagnosa:</strong> {{ $history->created_at->format('d M Y H:i') }}</p>
                            <p><strong>Gejala yang Dipilih:</strong></p>
                            <ul class="ml-3">
                                @php
                                    $selectedSymptoms = json_decode($history->selected_symptoms, true);
                                    $symptomNames = \App\Models\Symptom::whereIn('id', array_keys($selectedSymptoms))
                                        ->pluck('name')
                                        ->toArray();
                                @endphp
